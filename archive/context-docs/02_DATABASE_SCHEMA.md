# 02 — DATABASE SCHEMA
## MySQL 8.x — God of Ceramic Studio Management System
...
CREATE TABLE staff (
  ...
  staff_code      VARCHAR(20) UNIQUE NOT NULL,   -- GOC-STF-01
  full_name       VARCHAR(100) NOT NULL,
  phone           VARCHAR(15) UNIQUE NOT NULL,
  email           VARCHAR(100) UNIQUE,
  role            ENUM('owner','manager','detailer','sales') NOT NULL DEFAULT 'detailer',
...
CREATE TABLE customers (
  id              INT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
  customer_code   VARCHAR(20) UNIQUE NOT NULL,   -- GOC-CUST-0001
  full_name       VARCHAR(100) NOT NULL,
  phone           VARCHAR(15) UNIQUE NOT NULL,
...
CREATE TABLE vehicles (
  id              INT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
  vehicle_code    VARCHAR(20) UNIQUE NOT NULL,   -- GOC-VEH-0001
...
CREATE TABLE leads (
  id              INT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
  lead_code       VARCHAR(20) UNIQUE NOT NULL,   -- GOC-LEAD-0001
  full_name       VARCHAR(100) NOT NULL,
  phone           VARCHAR(15) NOT NULL,
  vehicle_make    VARCHAR(50),
  vehicle_model   VARCHAR(50),
  requirement     VARCHAR(255),                  -- comma-sep: ppf,ceramic,polish
  source          ENUM('facebook','instagram','whatsapp','walkin','reference','other') NOT NULL,
  connector_id    INT UNSIGNED NULL,
  assigned_to     INT UNSIGNED NULL,             -- staff.id
  status          ENUM('new','contacted','interested','quotation_sent','booked','lost') DEFAULT 'new',
  lost_reason     TEXT NULL,
  customer_id     INT UNSIGNED NULL,             -- linked after conversion
  notes           TEXT,
  fb_lead_id      VARCHAR(100) NULL,             -- Facebook Lead ID for dedup
  created_at      TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  updated_at      TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  deleted_at      TIMESTAMP NULL,
  FOREIGN KEY (connector_id) REFERENCES connectors(id) ON DELETE SET NULL,
...
CREATE TABLE lead_activity_log (
  ...
  action          VARCHAR(100) NOT NULL,         -- 'status_changed', 'note_added', 'whatsapp_sent'
...
CREATE TABLE bookings (
  id              INT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
  booking_code    VARCHAR(20) UNIQUE NOT NULL,   -- GOC-BKG-0001
  customer_id     INT UNSIGNED NOT NULL,
  ...
  assigned_staff  JSON,                          -- array of staff IDs
  status          ENUM('scheduled','cancelled','converted') DEFAULT 'scheduled',
  notes           TEXT,
...
CREATE TABLE job_cards (
  id              INT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
  job_code        VARCHAR(20) UNIQUE NOT NULL,   -- GOC-JC-0001
...
CREATE TABLE job_services (
  ...
  package_tier    ENUM('basic','premium','elite') DEFAULT 'basic',
  description     TEXT,
  sqft_used       DECIMAL(8,2) DEFAULT 0,        -- for PPF services
  ml_used         DECIMAL(8,2) DEFAULT 0,         -- for ceramic
...
CREATE TABLE quotations (
  id              INT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
  quotation_code  VARCHAR(20) UNIQUE NOT NULL,   -- GOC-QT-0001
  customer_id     INT UNSIGNED NOT NULL,
  vehicle_id      INT UNSIGNED NOT NULL,
  lead_id         INT UNSIGNED NULL,
  diagram_data    JSON NOT NULL,                  -- SVG zone selections
...
CREATE TABLE quotation_zones (
  ...
  zone_key        VARCHAR(50) NOT NULL,           -- 'bonnet', 'roof', 'door_fl', etc.
  zone_label      VARCHAR(50) NOT NULL,
  material_brand  VARCHAR(50),                    -- 3M, XPEL, SunTek
...
CREATE TABLE invoices (
  id                INT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
  invoice_code      VARCHAR(30) UNIQUE NOT NULL,  -- GOC-INV-2526-0001
  job_card_id       INT UNSIGNED NOT NULL,
  customer_id       INT UNSIGNED NOT NULL,
...
CREATE TABLE invoice_items (
  ...
  hsn_sac         VARCHAR(20) DEFAULT '998714',   -- Auto detailing SAC code
...
CREATE TABLE payments (
  ...
  reference_no    VARCHAR(100),                   -- UPI txn ID, cheque no, etc.
  notes           TEXT,
...
CREATE TABLE inventory_items (
  ...
  purchase_price  DECIMAL(10,2) DEFAULT 0,       -- per unit
  selling_price   DECIMAL(10,2) DEFAULT 0,
  location        VARCHAR(50),                    -- shelf/rack location
  notes           TEXT,
  created_at      TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  updated_at      TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  deleted_at      TIMESTAMP NULL
...
CREATE TABLE ppf_rolls (
  ...
  grade           VARCHAR(50),                    -- TPU, PVC, matte, gloss
  width_cm        DECIMAL(6,2) NOT NULL,
  length_m        DECIMAL(8,2) NOT NULL,
  total_sqft      DECIMAL(10,2) NOT NULL,         -- auto-calc: (width_cm/100 * length_m * 10.764)
  used_sqft       DECIMAL(10,2) DEFAULT 0,
  balance_sqft    DECIMAL(10,2) NOT NULL,         -- total - used
...
CREATE TABLE inventory_usage (
  ...
  total_deducted  DECIMAL(10,2) NOT NULL,         -- qty_used + wastage_qty
...
CREATE TABLE attendance (
  ...
  working_hours   DECIMAL(5,2),                   -- auto-calc on checkout
...
```sql
CREATE TABLE connector_commissions (
  id              INT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
...
CREATE TABLE notifications (
  id              INT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
  staff_id        INT UNSIGNED NOT NULL,
  type            VARCHAR(50) NOT NULL,           -- 'new_lead', 'car_ready', 'low_stock'
  title           VARCHAR(200) NOT NULL,
  body            TEXT,
  reference_type  VARCHAR(30),                    -- 'lead', 'job_card', 'inventory'
...
('studio_address', 'Vadodara, Gujarat, India', 'Studio address'),
('studio_phone', '+91XXXXXXXXXX', 'Contact number'),
('studio_gstin', 'GSTIN_HERE', 'GST number'),
